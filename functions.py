...
def find_linear(use_log=False): #Find the f(n) for which availability(f(n),n) is constant for all n
    goal=(log_availability if use_log else availability)(lambda x:2)(3)
    a=[0]*(MAXN+STEP+1)
    for i in range(MINN,MAXN+STEP+1):
        a[i]=math.floor(i/2)+1
        v=abs((log_availability if use_log else availability)(lambda x:math.floor(i/2)+1)(i)-goal)
        for j in range(math.floor(i/2)+1,i):
            x=abs((log_availability if use_log else availability)(lambda x:j)(i)-goal)
            if(x<v):
                v=x
                a[i]=j
